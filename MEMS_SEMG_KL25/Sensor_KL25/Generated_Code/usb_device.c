...
**     Date/Time   : 2015-05-21, 16:40, # CodeGen: 42
**     Abstract    :
**
**     Settings    :
**
...
extern     USB_CLASS_MEMS_ENDPOINT_DATA    g_mems_endpoint_data;
...
void USB_LDD_OnDeviceSof(LDD_TUserData *UserDataPtr, uint16_t FrameNumber)
{
  /* Write your code here ... */
		...
		if( ( G_FrameNumber == S_FrameNumber ) && ( first == 0 ) )
		    {
			   //(void)USB_Class_Mems_Send_Data(0, 1, wav_data,wav_size) ;
			    SwapARMDataBuffer();
				TransmitMCUData();
		...
		if( ( G_FrameNumber == S_FrameNumber ) && ( first == 1 ) && ( flag == 0 ) )
			{   
			    ...
			    if(1 != queue_num)//the USB sending queue is empty(not full),we can swap the buffer
			    {
			    	//(void)USB_Class_Mems_Send_Data(0, 1, wav_data,wav_size) ;
			    	SwapARMDataBuffer();
			    	TransmitMCUData();
				...
				if( G_FrameNumber >= 924 )//when frame number loop,S_FrameNumber is a little value(eg:52),then the G_FrameNumber(>2000) is more than S_FrameNumber
					flag = 1;
			}

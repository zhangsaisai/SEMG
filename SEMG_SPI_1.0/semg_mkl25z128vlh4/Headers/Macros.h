...
extern const uint8 SPI0_RX_DMA_CHANNEL;
     ...
     * @{
     */
#define ORDER 40                                            /*!< The order of the lpf. */
#define NTAPS 41                                            /*!< The number of coefficients. NTAP=ORDER+1. */
    /*!
     * @}
     */
    /*!
 ...
 *
   ...
   status_high: [7] - x
                ...
                [5] - x
                [4] - x
                [3:1] - Buffer Status(eEmpty, eWrite, eFull or eOverflow)
                [0] - Buffer Label(left buffer or right buffer)
   status_low: [7-0] - Overflow Count. If status_high[3:1] is eWrite, this byte means how many bytes are added.
    								   If status_high[3:1] is eOverflow, this byte means how many bytes are overflowed.
   @endverbatim
 */
...
typedef struct TFilterQueue TFilterBufferQueue;
typedef struct TFilterQueue* TFilterBufferQueuePtr;
/*!
...
struct TMCUData
{
    ...
    //int16 filteredBuffer[USING_ADC_COUNT][USING_CHANNEL_COUNT][NTAPS];                      /*!< The lpf buffer. */
    TFilterBufferQueue filterBufferQueue[USING_ADC_COUNT][USING_CHANNEL_COUNT];                        /*!< The lpf buffer queue. */
};

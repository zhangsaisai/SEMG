...
static void MCUDataInit(TMCUDataPtr userDataPtr)
{
    ...
    for(int adc = 0; adc < USING_ADC_COUNT; adc++)
    {
        for(int chn = 0; chn < USING_CHANNEL_COUNT; chn++)
        {
            for(int ntaps = 0; ntaps < NTAPS; ntaps++)
            {
                tMCU.mcuData.filterBufferQueue[adc][chn].buffer[ntaps] = 0;
            }
            tMCU.mcuData.filterBufferQueue[adc][chn].leftEdge = tMCU.mcuData.filterBufferQueue[adc][chn].buffer;
            tMCU.mcuData.filterBufferQueue[adc][chn].rightEdge = tMCU.mcuData.filterBufferQueue[adc][chn].buffer + NTAPS - 1;
            tMCU.mcuData.filterBufferQueue[adc][chn].head = tMCU.mcuData.filterBufferQueue[adc][chn].buffer;
            tMCU.mcuData.filterBufferQueue[adc][chn].tail = tMCU.mcuData.filterBufferQueue[adc][chn].buffer + NTAPS - 1;
            tMCU.mcuData.filterBufferQueue[adc][chn].len = NTAPS;
        }
    }

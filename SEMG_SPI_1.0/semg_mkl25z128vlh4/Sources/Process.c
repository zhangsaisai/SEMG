 ...
 * ===================================================================
 */
static int chDataCnt = 0;//How many data has been packed in buffer.When acquire the 100st data , chDataCnt clear to 0 at once.
static int chDataCnt_send = 0;//How many data has been packed in buffer.When acquire the 100st data , not clear to 0 until next 1ms.
...
static LDD_TError SplitRawData(void)
{
    ...
    for(int adcNum = 0; adcNum < USING_ADC_COUNT; adcNum++)
    {
        ...
        for(int i = 0; i < 16; i += 2)
        {
            ...
            if(eADC0 == adcNum)	
            {
                //According to the electrodes arrangement, the data from the 1st ADC should be reversed and be negetive.
            	//由于硬件设计的关系，当采集的是ADC0的数据时，需要把0-7通道的数据的顺序取反。
            	//又因为，0-7通道是负向差分，所以需要把0-7通道的值取反。
            	//按照运算规则，0x8000取反后的值会溢出，因此当值为0x8000时，强制定义为正的最大值0x7FFF。
...
#endif
#ifdef TEST
        int16 const (* const adc0ChannelDataPtr)[USING_CHANNEL_COUNT] = &(tADCPtr[0]->adcData.channelData);
        int16 const (* const adc1ChannelDataPtr)[USING_CHANNEL_COUNT] = &(tADCPtr[1]->adcData.channelData);
        int16 (* const mcuChannelDataPtr)[USING_ADC_COUNT][USING_CHANNEL_COUNT][CHANNEL_DATA_COUNT] = &(tMCUPtr->mcuData.channelData);
        TFilterBufferQueue (* const mcuFilterBufferQueuePtr)[USING_ADC_COUNT][USING_CHANNEL_COUNT] = &(tMCUPtr->mcuData.filterBufferQueue);
        
        *(*(*(*mcuChannelDataPtr)) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr, **mcuFilterBufferQueuePtr);
        *(*(*(*mcuChannelDataPtr) + 1) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 1, *(*mcuFilterBufferQueuePtr) + 1);
        *(*(*(*mcuChannelDataPtr) + 2) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 2, *(*mcuFilterBufferQueuePtr) + 2);
        *(*(*(*mcuChannelDataPtr) + 3) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 3, *(*mcuFilterBufferQueuePtr) + 3);
        *(*(*(*mcuChannelDataPtr) + 4) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 4, *(*mcuFilterBufferQueuePtr) + 4);
        *(*(*(*mcuChannelDataPtr) + 5) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 5, *(*mcuFilterBufferQueuePtr) + 5);
        *(*(*(*mcuChannelDataPtr) + 6) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 6, *(*mcuFilterBufferQueuePtr) + 6);
        *(*(*(*mcuChannelDataPtr) + 7) + chDataCnt) = EvenFirLPF(*adc0ChannelDataPtr + 7, *(*mcuFilterBufferQueuePtr) + 7);

        *(*(*(*mcuChannelDataPtr + 1)) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr, *(*mcuFilterBufferQueuePtr + 1));
        *(*(*(*mcuChannelDataPtr + 1) + 1) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 1, *(*mcuFilterBufferQueuePtr + 1) + 1);
        *(*(*(*mcuChannelDataPtr + 1) + 2) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 2, *(*mcuFilterBufferQueuePtr + 1) + 2);
        *(*(*(*mcuChannelDataPtr + 1) + 3) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 3, *(*mcuFilterBufferQueuePtr + 1) + 3);
        *(*(*(*mcuChannelDataPtr + 1) + 4) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 4, *(*mcuFilterBufferQueuePtr + 1) + 4);
        *(*(*(*mcuChannelDataPtr + 1) + 5) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 5, *(*mcuFilterBufferQueuePtr + 1) + 5);
        *(*(*(*mcuChannelDataPtr + 1) + 6) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 6, *(*mcuFilterBufferQueuePtr + 1) + 6);
        *(*(*(*mcuChannelDataPtr + 1) + 7) + chDataCnt) = EvenFirLPF(*adc1ChannelDataPtr + 7, *(*mcuFilterBufferQueuePtr + 1) + 7);
#endif
}
...
//    static int chDataCnt[USING_ADC_COUNT] = {0};
    ...
    if(chDataCnt < CHANNEL_DATA_COUNT)//if chDataCnt more than 100,chDataCnt still remain 100 until swap buffer
    	chDataCnt++;
    //chDataCnt_send = chDataCnt;
...
//        tARMPtr->armStatus.isForeBufferFull = FALSE;
    ...
    else
    {
        ...
        //Set the new status of the 2 buffers.
...
//                	    + 3 * (channelNum + 1)                                    /* Every channel's data pack has a head of 3 bits. */
//                	    + CHANNEL_DATA_COUNT * 2 * channelNum                     /* every channel's data is 200 bits apart of the 3-bit head. */
//                	    + (remainNum - j) * 2;                                    /* every channel's data at one time is 2 bits. */

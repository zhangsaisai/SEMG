...
**     Date/Time   : 2015-01-06, 20:08, # CodeGen: 286
**     Abstract    :
...
uint16_t      G_FrameNumber;// frame number global variable
uint16_t      S_FrameNumber = 10000;//expected frame number
//uint8_t       FrameNumber_H;
//uint8_t       FrameNumber_L;
...
static void USB_LDD_DeviceIsr(USB_LDD_TDeviceData *DevDataPtr, uint32_t InSignalsState)
{
  ...
  if (DevDataPtr->DeviceState >= LDD_USB_DEVICE_VBUS_OFF) { /* Device enabled and VBUS = ON? */
    ...
    if ((IntStatus & USB_PDD_SOF_TOK_INT) != 0x00U) {       /* SOF received? */
      ...
      G_FrameNumber = FrameNumber & 0X3FF;
      if(isStart){
         if(tARMPtr->foreBuffer[6] >= 255)
        	 tARMPtr->foreBuffer[6] = 255;
         else
        	 tARMPtr->foreBuffer[6] ++;//If forebuffer not init,SOF foreBuffer[6]++ lead to hard fault;
      }
    ...
    if ((IntStatus & USB_PDD_TOK_DNE_INT) != 0x00U) {
      ...
      if (USB_PDD_GetTransmitIndicator(USB0_BASE_PTR)) {    /* Tx done? */
    	  //EpDataPtr->pCurrentTD->BufferPtr[4] = FrameNumber_H;
    	  //EpDataPtr->pCurrentTD->BufferPtr[5] = FrameNumber_L;
    	  //EpDataPtr->pCurrentTD->BufferPtr[6] = wait_time;

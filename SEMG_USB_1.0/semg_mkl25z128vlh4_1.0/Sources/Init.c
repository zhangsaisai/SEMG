...
static void ARMDataInit(TARMDataPtr userDataPtr)
{
    ...
    tARM.armDataLeft.dataFrame[2] = 0x00U;                                      /* Data length high bits */
    tARM.armDataLeft.dataFrame[3] = 0x00U;                                      /* Data length low bits */
    tARM.armDataLeft.dataFrame[4] = 0x00U;                                      /* frame number high bits */
    tARM.armDataLeft.dataFrame[5] = 0x00U;                                      /* frame number low bits */
...
/*
    for(int i = 0; i < USING_CHANNEL_COUNT * USING_ADC_COUNT; i++)
    {
        ...
        tARM.armDataLeft.dataFrame[9 + off] = CHANNEL_PACKAGE_HEAD_BIT;          
        tARM.armDataLeft.dataFrame[10 + off] = MCU_NUMBER * USING_CHANNEL_COUNT * USING_ADC_COUNT + i;   
        tARM.armDataLeft.dataFrame[11 + off] = 0x00U;                            
    }
...
*/
    ...
    tARM.armDataRight.dataFrame[3] = 0x00U;                                      /* Data length low bits */
...
/*    
    ...
    {
        int off = i * CHANNEL_PACKAGE_LENGTH;
        tARM.armDataRight.dataFrame[9 + off] = CHANNEL_PACKAGE_HEAD_BIT;                   
        tARM.armDataRight.dataFrame[10 + off] = MCU_NUMBER * USING_CHANNEL_COUNT * USING_ADC_COUNT + i;  
        tARM.armDataRight.dataFrame[11 + off] = 0x00U;                                     
    }
...
void ARMDataHeaderInit(void)
{
    ...
    tARM.armDataLeft.dataFrame[2] = (uint8)(32*Period+48)>>8;                   /* Data length high bits */
    tARM.armDataLeft.dataFrame[3] = (uint8)(32*Period+48);                      /* Data length low bits */
    
    for(int i = 0; i < USING_CHANNEL_COUNT * USING_ADC_COUNT; i++)
    {
        int off = i * CHANNEL_PACKAGE_LENGTH;
        tARM.armDataLeft.dataFrame[9 + off] = CHANNEL_PACKAGE_HEAD_BIT;          /* Channel package head bit */
        tARM.armDataLeft.dataFrame[10 + off] = MCU_NUMBER * USING_CHANNEL_COUNT * USING_ADC_COUNT + i;   /* Channel number */
        tARM.armDataLeft.dataFrame[11 + off] = 0x00U;                            /* Channel package status */
    }
    
    tARM.armDataRight.dataFrame[2] = (uint8)(32*Period+48)>>8;                 /* Data length high bits */
    tARM.armDataRight.dataFrame[3] = (uint8)(32*Period+48);                    /* Data length low bits */    
    
    for(int i = 0; i < USING_CHANNEL_COUNT * USING_ADC_COUNT; i++)
    {
        int off = i * CHANNEL_PACKAGE_LENGTH;
        tARM.armDataRight.dataFrame[9 + off] = CHANNEL_PACKAGE_HEAD_BIT;                   /* Channel package head bit */
        tARM.armDataRight.dataFrame[10 + off] = MCU_NUMBER * USING_CHANNEL_COUNT * USING_ADC_COUNT + i;  /* Channel number */
        tARM.armDataRight.dataFrame[11 + off] = 0x00U;                                     /* Channel package status */
    }
